...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, leaky_relu=False, leaky_scale=0, **kwargs):
      ...
      #  self.stage4_cfg['NUM_CHANNELS'] = [40, 80, 160, 320]
      ...
      #      self.stage4_cfg, num_channels, multi_scale_output=True)
        ...
        #    nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
            nn.Conv2d(
                ...
                out_channels=128,
                kernel_size=3, # using 3 for DGANet
                stride=1,
                padding=1),
    ...
    def forward(self, x):
     ...
     #   x = self.stage4(x_list)
        ...
        x1 = F.upsample(x[1], size=(x0_h, x0_w), mode='bilinear', align_corners=False)
        x2 = F.upsample(x[2], size=(x0_h, x0_w), mode='bilinear', align_corners=False)
    #    x3 = F.upsample(x[3], size=(x0_h, x0_w), mode='bilinear', align_corners=False)
       ...
       # x = torch.cat([x[0], x1, x2, x3], 1)
        ...
        x = nn.functional.interpolate(x, size=(x0_h*2, x0_w*2), mode='bilinear', align_corners=False)
